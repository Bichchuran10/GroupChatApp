...
//console.log("the soket is",socket)
...
const updateUserList = async () => {
    ...
    const userList = document.getElementById('userList')
    const users = await axios.get('http://localhost:3000/user/get-users', { headers: { "Authorization": token } })
    console.log('inside updateUserList usersss',users)
    for (let user of users.data.users) {
        const liElement = document.createElement('li')
        liElement.className = 'user'
        liElement.innerHTML +=
        ...
        <div class="user-info">
            <h4 class="user-name" id=user_token_no_${userTokenNumber}>${user.name}</h4>
...
const addListners = () => {
    ...
    console.log("inside addListners",listOfUserTokens)
    ...
    for (let i = 0; i < listOfUserTokens.length; i++) {
        try {
            document.getElementById(listOfUserTokens[i]).addEventListener('click', async () => {
                ...
                console.log("cuuuurrr usssrr",currentUserId)

                console.log('from local storageeee',currentUserId, parseInt(localStorage.getItem('currentTextingPerson')))
...
const loadPreviousChats = async (userId) => {
    try {
        console.log('inside load prev chats',userId)
        let recentReceivedChat
        const token = localStorage.getItem('token')
